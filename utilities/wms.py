r.in.wms --overwrite url=https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/1.0.0/WMTSCapabilities.xml output=blue_marble layers=BlueMarble_NextGeneration format=jpeg srs=3857 driver=WMTS_GRASS


https://wiki.earthdata.nasa.gov/display/GIBS/GIBS+API+for+Developers#GIBSAPIforDevelopers-OGCWebMapTileService(WMTS)

https://wiki.earthdata.nasa.gov/display/GIBS/GIBS+Available+Imagery+Products#expand-ReferenceLayers9Layers

http://www.globalclimatemonitor.org/#

https://gis.ucar.edu/data/climate/wms
